...
--   * 成果は「注文ID」を主キーにしているので、再取り込みでは行が増えず
--     ステータスなどの変化だけが上書きされる。
...
-- 成果データの取り込み（分割送信できるよう 1 チャンクぶんだけ処理する）。
...
begin
  with src as (
    ...
    select
      s.order_id, s.occurred_at, s.product_id, s.advertiser_id, s.product_name, s.qty, s.unit_price,
      s.affiliate_id, s.reward, s.reward_rate, s.pay_status1, s.tier2_id, s.tier2_reward, s.tier2_rate,
      s.pay_status2, s.sale_price, s.campaign, s.ad_name, s.status, s.pay_status, s.billed_on,
      s.last_click_at, s.last_referrer, s.first_click_at, s.first_referrer, s.device, s.os,
      s.status_changed_at, s.memo, p_file_name
    from src s
  ...
  return jsonb_build_object(
    'total', v_total, 'inserted', v_inserted, 'updated', v_updated, 'skipped', 0);
end $$;
...
-- 任意の軸で集計する。
-- p_dim: product / ad / campaign / advertiser / device / os / referrer /
--        ad_type / status / pay_status / affiliate
-- クリック側に無い軸（商品・デバイス・ステータス）はクリック列を 0 に、
-- 成果側に無い軸（広告タイプ）は成果列を 0 にして返す。
